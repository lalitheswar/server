	...
	dict_table_t*	table)
{
	...
	sync_table = static_cast<dict_table_t*>(
			mem_heap_alloc(msg->heap, sizeof(dict_table_t)));
	sync_table = table;
	msg->ptr = sync_table;
...
/*===================*/
{
	...
	/* There is no need to add the fts tables to fts_optimize_wq
	queue here because loaded fts tables will be loaded during
	fts_init_index(). */
